...
app.put('/v1/music/tracks/:trackId/schedule', async (req, res) => {
    try {
        ...
        await trackRef.update({
            scheduledReleaseDate: firebase_admin_1.default.firestore.Timestamp.fromDate(releaseDateObj),
            status: 'scheduled',
...
app.put('/v1/music/tracks/:trackId/lyrics', async (req, res) => {
    try {
        ...
        const updates = { updatedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp() };
        if (lyrics)
            updates.lyrics = lyrics;
        if (syncedLyrics)
            updates.syncedLyrics = syncedLyrics;
...
app.post('/v1/music/artists/:artistId/analytics/export', async (req, res) => {
    try {
        ...
        tracksSnap.docs.forEach(doc => {
            var _a;
            const data = doc.data();
            csvContent += `${doc.id},"${data.title || ''}",${data.streamCount || 0},${data.likeCount || 0},${data.status || ''},${((_a = data.createdAt) === null || _a === void 0 ? void 0 : _a.toDate().toISOString()) || ''}\n`;
        });
