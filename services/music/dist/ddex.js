...
exports.buildERNMessage = buildERNMessage;
...
function buildERNMessage(release) {
    ...
    const soundRecordings = release.tracks
      ...
      <SoundRecording>
        ...
        <PLine><Year>${esc(t.pLineYear || String(new Date().getFullYear()))}</Year><PLineText>${esc(t.pLineText || t.artistName)}</PLineText></PLine>
        <TechnicalDetails>
...
<ern:NewReleaseMessage xmlns:ern="http://ddex.net/xml/ern/43" MessageSchemaVersionId="ern/43" LanguageAndScriptCode="en">
  ...
  <ReleaseList>
    <Release>
      ...
      <PLine><Year>${esc(release.pLineYear || String(new Date().getFullYear()))}</Year><PLineText>${esc(release.pLineText || release.displayArtist)}</PLineText></PLine>
      <CLine><Year>${esc(release.cLineYear || String(new Date().getFullYear()))}</Year><CLineText>${esc(release.cLineText || release.label || release.displayArtist)}</CLineText></CLine>
