...
app.post('/v1/music/artists/:artistId/tours', async (req, res) => {
    try {
        ...
        await tourRef.set({
            ...
            artistId,
            name: name.trim(),
            startDate: startDate ? firebase_admin_1.default.firestore.Timestamp.fromDate(new Date(startDate)) : null,
            endDate: endDate ? firebase_admin_1.default.firestore.Timestamp.fromDate(new Date(endDate)) : null,
            description: description || null,
            status: 'upcoming',
            createdAt: now
        });
...
app.post('/v1/music/tours/:tourId/dates', async (req, res) => {
    try {
        ...
        await tourDateRef.set({
            ...
            venue,
            city,
            date: firebase_admin_1.default.firestore.Timestamp.fromDate(new Date(date)),
    ...
    try {
        ...
        const tours = toursSnap.docs.map(doc => {
            ...
            return {
                id: doc.id,
                name: data.name,
                startDate: (_a = data.startDate) === null || _a === void 0 ? void 0 : _a.toDate().toISOString(),
                endDate: (_b = data.endDate) === null || _b === void 0 ? void 0 : _b.toDate().toISOString(),
                status: data.status
            };
        });
        res.json({
