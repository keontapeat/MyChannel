...
exports.deliverRelease = deliverRelease;
...
async function deliverViaAggregator(assets) {
    if (!API_BASE || !API_KEY) {
        return {
            provider: PROVIDER,
            deliveryId: '',
 ...
 */
async function deliverViaDDEXSFTP(assets) {
...
async function deliverMock(assets) {
    return {
        provider: 'mock',
        deliveryId: `mock_${assets.upc}_${Date.now()}`,
        status: 'submitted',
        message: 'Mock delivery accepted. Set DISTRIBUTION_PROVIDER + AGGREGATOR_API_* to deliver to real DSPs.',
    };
}
async function deliverRelease(assets) {
    switch (PROVIDER) {
        case 'fuga':
        case 'revelator':
        case 'sonosuite':
