 ...
 */
export async function allocateISRC(): Promise<string> {
  ...
  const designation = await db().runTransaction(async (tx) => {
    ...
    tx.set(counterRef, { value: next, updatedAt: admin.firestore.Timestamp.now() }, { merge: true });
    return next;
  });
 ...
 */
export async function allocateUPC(): Promise<string> {
...
export function isValidUPC(upc: string): boolean {
  ...
  if (compact.length !== 13) return false;
