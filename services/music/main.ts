...
const MAX_REQUESTS_PER_MINUTE = Math.min(
  Math.max(Number.parseInt(process.env.MUSIC_RATE_LIMIT_PER_MINUTE || '120', 10) || 120, 10),
  1000
);
...
const TRANSCODE_HANDOFF_TIMEOUT_MS = Math.min(
  Math.max(Number.parseInt(process.env.MUSIC_TRANSCODE_HANDOFF_TIMEOUT_MS || '600000', 10) || 600_000, 5_000),
  900_000
);
const TOTAL_SPLIT_BASIS_POINTS = 10_000;
...
const MAX_QUALIFIED_PLAY_SECONDS = 86_400;
const MAX_QUALIFIED_PLAYS_PER_TRACK_DAY = Math.min(
  Math.max(Number.parseInt(process.env.MUSIC_MAX_QUALIFIED_PLAYS_PER_TRACK_DAY || '50', 10) || 50, 1),
  500
);
  ...
  if (!current || current.resetAt <= now) {
    ...
    if (rateWindows.size > 10_000) {
      for (const [windowKey, value] of rateWindows) {
        if (value.resetAt <= now) rateWindows.delete(windowKey);
      }
    }
    return next();
  }
...
function safeAudioType(value: unknown): string {
...
function safeArtworkType(value: unknown): string {
...
function configuredTranscodeEndpoint(trackId: string): URL | null {
...
async function enqueueConfiguredTranscode(
  trackId: string,
  ownerUid: string
): Promise<{ configured: boolean; accepted: boolean; statusCode?: number }> {
  ...
  const taskId = `music-transcode-${createHash('sha256').update(trackId).digest('hex')}`;
...
async function invokeConfiguredTranscode(
  trackId: string,
  authorization: string
): Promise<{ configured: boolean; accepted: boolean; statusCode?: number }> {
  const endpoint = configuredTranscodeEndpoint(trackId);
  if (!endpoint) return { configured: false, accepted: false };
  ...
  return new Promise((resolve, reject) => {
    const request = httpsRequest(endpoint, {
      ...
      headers: {
        ...
        'X-MyChannel-Idempotency-Key': createHash('sha256')
      ...
      response.on('data', (chunk: Buffer) => {
        ...
        if (responseBytes > 64 * 1024) request.destroy(new Error('Transcode response exceeded limit'));
      });
      response.on('end', () => {
...
async function handoffCompletedUpload(
  trackId: string,
  ownerUid: string,
  trackRef: FirebaseFirestore.DocumentReference,
  processingRef: FirebaseFirestore.DocumentReference,
  authorization: string
): Promise<{ httpStatus: number; status: string; transcodingStatus: string; message: string }> {
  try {
    const queued = await enqueueConfiguredTranscode(trackId, ownerUid);
    const allowSynchronousFallback =
      (process.env.MUSIC_ALLOW_SYNCHRONOUS_TRANSCODE || 'false') === 'true';
    ...
    await processingRef.set({
      ...
      handoffStatusCode: handoff.statusCode || null,
      handoffAcceptedAt: admin.firestore.FieldValue.serverTimestamp(),
      updatedAt: admin.firestore.FieldValue.serverTimestamp()
    }, { merge: true });
  ...
  try {
    ...
    try {
      uploadedChunks = await db.runTransaction(async (transaction) => {
        ...
        if (!current.exists) throw new Error('Track not found');
        ...
        const indexes: number[] = Array.isArray(data.uploadedChunkIndexes)
          ? data.uploadedChunkIndexes.filter((value: unknown) => Number.isSafeInteger(value))
          : [];
        if (!indexes.includes(chunkIndex)) indexes.push(chunkIndex);
...
app.post('/v1/music/tracks/:trackId/complete', async (req, res) => {
  try {
    ...
    if (trackData.status === 'processing') {
      ...
      const handoff = await handoffCompletedUpload(
        trackId,
        user.userId,
        trackRef,
        processingRef,
    ...
    const chunkDetails = await Promise.all(chunks.map(async (file) => {
      ...
      return {
        ...
        ownerUid: metadata.metadata?.ownerUid,
        trackId: metadata.metadata?.trackId,
        totalChunks: Number(metadata.metadata?.totalChunks)
      };
    }));
    ...
    const generatedIntermediates: any[] = [];
    if (chunkDetails.length === 1) {
      ...
      while (intermediates.length > 1) {
        ...
        for (let index = 0; index < intermediates.length; index += 32) {
          ...
          if (target.name !== finalAudioPath) generatedIntermediates.push(target);
          next.push(target);
        }
        intermediates = next;
        wave += 1;
...
app.get('/v1/music/tracks', async (req, res) => {
  try {
    ...
    let query: FirebaseFirestore.Query = db.collection('music_tracks')
    ...
    if (pageToken) {
      ...
      if (!cursor.exists || String(cursor.data()?.artistId || '') !== user.userId) {
        return res.status(400).json({ error: 'Invalid pageToken' });
      }
...
app.put('/v1/music/tracks/:trackId/moderation', async (req, res) => {
  try {
    const user = await requireUser(req, res);
    if (!user) return;
    if (!user.isAdmin) return res.status(403).json({ error: 'Admin authorization required' });
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (!trackSnap.exists) return { error: 'Track not found', statusCode: 404 };
      ...
      if (decision === 'approved') update.publishedAt = now;
...
app.put('/v1/music/tracks/:trackId/publish', async (req, res) => {
  try {
    ...
    const result = await db.runTransaction(async (transaction) => {
      const trackSnap = await transaction.get(trackRef);
      if (!trackSnap.exists) return { error: 'Track not found', status: 404 };
      const trackData = trackSnap.data()!;
      if (String(trackData.artistId || '') !== user.userId) {
...
app.post('/v1/music/tracks/:trackId/plays', async (req, res) => {
  try {
    ...
    const qualifiedSeconds = parseBoundedInteger(
      req.body.qualifiedSeconds,
      'qualifiedSeconds',
      MIN_QUALIFIED_PLAY_SECONDS,
      MAX_QUALIFIED_PLAY_SECONDS
    );
    const sessionId = req.body.sessionId;
    const utcDay = new Date().toISOString().slice(0, 10);
    const playId = createHash('sha256')
      .update(`${user.userId}\u0000${trackId}\u0000${sessionId}`)
      .digest('hex');
    const dailyLimitId = createHash('sha256')
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (!artistId) throw new Error('Published track has no artist owner');
...
app.get('/v1/music/tracks/:trackId/collaborators', async (req, res) => {
  try {
    ...
    const collaborators = split.collaborators.map((collaborator: any) => ({
...
app.put('/v1/music/tracks/:trackId/collaborators', async (req, res) => {
  try {
    ...
    const collaborators = req.body.collaborators.map((value: unknown, index: number) => {
      ...
      const basisPoints = parseBoundedInteger(
        value.basisPoints,
        `collaborators[${index}].basisPoints`,
        1,
        TOTAL_SPLIT_BASIS_POINTS
      );
      return {
        id: value.artistId,
    ...
    const totalBasisPoints = collaborators.reduce(
      (sum: number, collaborator: { revenueShareBasisPoints: number }) =>
        sum + collaborator.revenueShareBasisPoints,
      0
    );
    ...
    if (!authorizationSnap.exists) return res.status(404).json({ error: 'Track not found' });
    if (String(authorizationSnap.data()?.artistId || '') !== user.userId) {
      return res.status(403).json({ error: 'Forbidden' });
    }

    const artistLookup = await admin.auth().getUsers(
      collaborators.map((collaborator) => ({ uid: collaborator.artistId }))
    );
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (ownerArtistId !== user.userId) return { error: 'Forbidden', status: 403 };
      if (!collaborators.some((collaborator) =>
        collaborator.artistId === ownerArtistId && collaborator.role === 'primary_artist')) {
        return { error: 'The track owner must be included as primary_artist', status: 400 };
      }
      if (payoutLockSnap.exists && payoutLockSnap.data()?.status === 'pending') {
        return { error: 'Collaborator splits cannot change during a pending payout', status: 409 };
      }

      const previousRevision = splitSnap.exists ? splitSnap.data()?.revision : 0;
      ...
      transaction.set(splitRef, {
        ...
        createdAt: splitSnap.exists
          ? splitSnap.data()?.createdAt || admin.firestore.FieldValue.serverTimestamp()
...
app.post('/v1/music/tracks/:trackId/likes', async (req, res) => {
  try {
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (likeSnap.exists) return { created: false, likeCount: currentLikeCount };
