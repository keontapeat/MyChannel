...
///
/// Managers are created lazily on first use so cold launch never prompts for
/// Bluetooth (Guideline 5.1.1) and never constructs CB* objects unless needed.
@MainActor
final class LocalBroadcastEngine: NSObject, ObservableObject {
    ...
    private override init() {
        super.init()
        // Intentionally do NOT create CBPeripheralManager / CBCentralManager here.
    }
...
extension LocalBroadcastEngine: CBCentralManagerDelegate {
    nonisolated func centralManagerDidUpdateState(_ central: CBCentralManager) {
        // Do not auto-start scanning — wait for an explicit startScanningForNearbyUsers().
    }
