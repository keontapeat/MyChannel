...
class APIService: ObservableObject {
    ...
    // MARK: - User API
    func authenticateUser(email: String, password: String) async throws -> UserProfile {
        try await Task.sleep(nanoseconds: 2_000_000_000)
        
        let mockUser = User.sampleUsers[0]
        // Convert to UserProfile shape expected elsewhere if needed
        return UserProfile(
            id: mockUser.id,
            ...
            bio: nil, websiteUrl: nil, location: nil, profileVisibility: nil, allowComments: nil, allowMessages: nil, preferredLanguage: nil, timezone: nil, emailNotifications: nil, pushNotifications: nil, lastActiveAt: nil
        )
    }
