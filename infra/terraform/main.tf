...
# Allow the Cloud Run runtime service account to sign V4 URLs
resource "google_service_account_iam_member" "run_sa_token_creator" {
  service_account_id = "projects/${var.project_id}/serviceAccounts/${data.google_service_account.run_svc.email}"
...
resource "google_storage_bucket_iam_member" "ingest_object_admin_run_svc" {
