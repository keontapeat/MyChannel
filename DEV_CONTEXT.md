...
gcloud builds submit --config services/cloudbuild-auth.yaml services
gcloud builds submit --config services/cloudbuild-search.yaml services
gcloud builds submit --config services/cloudbuild-moderation.yaml services
gcloud builds submit --config services/cloudbuild-creator.yaml services
```
...
## CI/CD
- GitHub Actions:
  - `.github/workflows/hosting.yml`: Deploys Firebase Hosting on PR (preview) and `main` (live clone)
  - `.github/workflows/functions.yml`: Deploys Firebase Functions (Python 3.12) on `main`
  - `.github/workflows/cloudrun-services.yml`: Builds and deploys all Cloud Run services on `main` via Cloud Build
- Required GitHub secrets:
  - `FIREBASE_TOKEN` (CI token with deploy access to project `mychannel-ca26d`)
  - `GCP_WORKLOAD_IDP` (OIDC provider resource path for Workload Identity Federation)
  - `GCP_DEPLOY_SA` (Service account email with `roles/run.admin`, `roles/cloudbuild.builds.editor`, `roles/artifactregistry.admin`)

## Ready-to-run commands (local)
```bash
# Hosting preview (safe):
firebase hosting:channel:deploy preview-$(date +%Y%m%d-%H%M%S) --project mychannel-ca26d --non-interactive

# Functions deploy:
firebase deploy --only functions --project mychannel-ca26d --non-interactive

# Cloud Run builds (submit via Cloud Build):
gcloud builds submit --config services/cloudbuild-auth.yaml services
gcloud builds submit --config services/cloudbuild-upload.yaml services
gcloud builds submit --config services/cloudbuild-transcode.yaml services
gcloud builds submit --config services/cloudbuild-content.yaml services
