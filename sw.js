...
const CACHE_NAME = 'mychannel-v1.0.36';
const STATIC_CACHE = 'mychannel-static-v31';
const DYNAMIC_CACHE = 'mychannel-dynamic-v31';

// Files to cache for offline functionality
const STATIC_FILES = [
    // '/',   // do not pre-cache root HTML to avoid stale shell
    '/app.html',
    '/auth.html',
    '/dashboard.html',
    '/upload.html',
    ...
    '/assets/js/creator-studio.js',
    '/assets/js/creator-tools.js',
...
// Prefer cache-first for TMDB images
    ...
    event.waitUntil(
        (async () => {
            try {
                ...
                console.warn('Navigation preload not available', e);
            }
...
self.addEventListener('fetch', (event) => {
    ...
    // Skip API calls that are failing to prevent infinite loops
    if (url.hostname.includes('gateway.dev') || url.hostname.includes('googleapis.com')) {
        event.respondWith(fetch(request).catch(() => new Response('', {status: 404})));
        return;
    }
    
    // Bypass caching for media with Range requests and GCS hosted media
    if (request.headers.get('range') || url.hostname.endsWith('storage.googleapis.com') || url.pathname.endsWith('.m3u8') || url.pathname.endsWith('.mpd') || url.pathname.endsWith('.ts') || url.pathname.endsWith('.mp4') || url.pathname.endsWith('.webm')) {
        event.respondWith(fetch(request));
        return;
    }

    event.respondWith((async () => {
        // Try navigation preload response first for navigations
